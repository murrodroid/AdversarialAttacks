...
class ModelRegistry:
    ...
    def load_model(cls, model_name: str, device: torch.device):
        ...
        if model_name not in cls._MODELS:
            raise ValueError(
                f"Model '{model_name}' not recognized. Available: {cls.get_available_models()}"
            )
...
class DatasetRegistry:
    ...
    def get_dataset_instance(cls, dataset_name: str):
        ...
        if dataset_name not in cls._DATASETS:
            raise ValueError(
                f"Dataset '{dataset_name}' not recognized. Available: {cls.get_available_datasets()}"
            )
...
class AttackRegistry:
    ...
    def get_attack_function(cls, attack_name: str) -> Callable:
        ...
        if attack_name not in cls._ATTACKS:
            raise ValueError(
                f"Attack '{attack_name}' not recognized. Available: {cls.get_available_attacks()}"
            )
