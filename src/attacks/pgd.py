...
def pgd_attack(model: object, source_image: Tensor, target_class: list, epsilon=0.3, alpha=0.01, max_iter=100, break_early: bool = False):
    perturbed_image = source_image.clone().detach() + torch.empty_like(source_image).uniform_(-epsilon/2, epsilon/2)
    perturbed_image = torch.clamp(perturbed_image, source_image.min(), source_image.max())
    ...
    for i in range(max_iter):
        ...
        for j in range(B):
              if pred_classes[j].item() == target[j].item():
                    if not success[j]:
                        ...
                        first_success_output[j] = probs[j].detach().clone().cpu().numpy().tolist()
                    success[j] = True
        ...
        perturbed_image = perturbed_image.detach() - alpha * torch.sign(perturbed_image.grad)
        eta = torch.clamp(perturbed_image - source_image, -epsilon, epsilon)
        perturbed_image = torch.clamp(source_image + eta, source_image.min(), source_image.max())
